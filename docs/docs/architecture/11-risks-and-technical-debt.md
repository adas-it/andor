...
| R-4 | **No verification-code expiry in Onboarding.** A code is only invalidated by a new `start`; there is no TTL. | Low–Medium | Add a time-based expiry and rate-limiting on `start`. Tracked on the [Onboarding page](../domains/onboarding.md). |
